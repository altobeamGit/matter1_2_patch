...
CHIP_ERROR ATBMP256Keypair::Initialize(ECPKeyTarget keyTarget, int efuseBlock)
{
    Clear();
    ...
    status = atbm_ecdsa_privkey_load_mpi(&ecdsa_ctx->MBEDTLS_PRIVATE(d), efuseBlock);
    VerifyOrExit(status == 0, error = CHIP_ERROR_INTERNAL);
...
CHIP_ERROR ATBMP256Keypair::ECDSA_sign_msg(const uint8_t * msg, const size_t msg_length, P256ECDSASignature & out_signature) const
{
