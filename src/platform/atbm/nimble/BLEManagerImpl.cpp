...
#include <platform/atbm/BLEManagerImpl.h>
...
CHIP_ERROR BLEManagerImpl::_Init()
{
	...
	ChipLogProgress(DeviceLayer, "BLE ManagerImpl Init---");
...
bool BLEManagerImpl::CloseConnection(BLE_CONNECTION_OBJECT conId)
{
    ...
    // Signal the ATBM BLE layer to close the conneecction.
...
{
	ChipLogProgress(DeviceLayer, "GetMTU start");
                                    ...
                                    chip::System::PacketBufferHandle data)
{
    ...
    ChipLogProgress(DeviceLayer, "Sending indication for CHIPoBLE TX characteristic (con %u, len %u)", conId, data->DataLength());
	...
	ChipLogProgress(DeviceLayer, "Sending indication custom");
...
void BLEManagerImpl::DriveBLEState(void)
{
    ...
    // Initializes the ATBM BLE layer if needed.
    if (mServiceMode == ConnectivityManager::kCHIPoBLEServiceMode_Enabled && !mFlags.Has(Flags::kATBMBLELayerInitialized))
    {
        err = InitATBMBleLayer();
        SuccessOrExit(err);
    ...
    )
    {
        ...
        // ATBM BLE layer needs to be refreshed.
        if (!mFlags.Has(Flags::kAdvertising) || mFlags.Has(Flags::kAdvertisingRefreshNeeded))
        {
            ...
            ChipLogProgress(DeviceLayer, "NimBLE start advertising...");
    ...
    else // Otherwise stop advertising if needed...
    {
        if (mFlags.Has(Flags::kAdvertising))
        {
        	 ChipLogProgress(DeviceLayer, "Check BLE gap advertising Active...");
...
{
    ChipLogProgress(DeviceLayer, "Resetting state; reason=%d\n", reason);
}
...
CHIP_ERROR BLEManagerImpl::bleprph_set_random_addr(void)
{
	...
	ChipLogProgress(DeviceLayer, "Generates a new static random address...");
    // Generates a new static random address
    int rc = ble_hs_id_gen_rnd(0, &addr);
    if (rc != 0)
    {
        ChipLogError(DeviceLayer, "Failed to generate random address err: %d", rc);
        return CHIP_ERROR_INTERNAL;
    }

	ChipLogProgress(DeviceLayer, "Set generated address...");
    // Set generated address
    rc = ble_hs_id_set_rnd(addr.val);
    if (rc != 0)
    {
        ChipLogError(DeviceLayer, "Failed to set random address err: %d", rc);
        return CHIP_ERROR_INTERNAL;
    }

	ChipLogProgress(DeviceLayer, "Try to configure the device with random static address...");
    // Try to configure the device with random static address
    rc = ble_hs_util_ensure_addr(1);
    if (rc != 0)
    {
        ChipLogError(DeviceLayer, "Failed to configure random address err: %d", rc);
        return CHIP_ERROR_INTERNAL;
    }
...
{
    ChipLogProgress(DeviceLayer, "BLE Host Task Started...");
    /* This function will return only when nimble_port_stop() is executed */
    nimble_port_run();
    //nimble_port_freertos_deinit();
}

CHIP_ERROR BLEManagerImpl::InitATBMBleLayer(void)
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    VerifyOrExit(!mFlags.Has(Flags::kATBMBLELayerInitialized), /* */);

    semaphoreHandle = xSemaphoreCreateBinary();
    if (semaphoreHandle == NULL)
    {
        err = CHIP_ERROR_NO_MEMORY;
        ChipLogError(DeviceLayer, "Failed to create semaphore");
        ExitNow();
    }
	...
	ChipLogProgress(DeviceLayer, "BLE nim port init...");
    nimble_port_init();
    ...
    // Register the CHIPoBLE GATT attributes with the ATBM BLE layer if needed.
    if (mServiceMode == ConnectivityManager::kCHIPoBLEServiceMode_Enabled)
    {
    	ChipLogProgress(DeviceLayer, "BLE svc gap&gatt init...");
        ble_svc_gap_init();
        ble_svc_gatt_init();
		...
		ChipLogProgress(DeviceLayer, "BLE gatts add svcs...");
	...
	ChipLogProgress(DeviceLayer, "BLE nimble_port_freertos_init start");
    nimble_port_freertos_init(bleprph_host_task);
    xSemaphoreTake(semaphoreHandle, portMAX_DELAY);
    vSemaphoreDelete(semaphoreHandle);
    semaphoreHandle = NULL;

    sInstance.mFlags.Set(Flags::kATBMBLELayerInitialized);
...
CHIP_ERROR BLEManagerImpl::ConfigureAdvertisingData(void)
{
	...
	ChipLogProgress(DeviceLayer, "Configure the BLE device name...");
    ...
    advData[index++] = kServiceDataTypeSize + sizeof(ATBMChipServiceData);                 // length
	...
	ChipLogProgress(DeviceLayer, "Construct the Chip BLE Service Data");
...
void BLEManagerImpl::HandleRXCharWrite(struct ble_gatt_char_context * param)
{
    ...
    ChipLogProgress(DeviceLayer, "Write request received for CHIPoBLE RX characteristic con %u %u", param->conn_handle, param->attr_handle);
	...
	ChipLogProgress(DeviceLayer, "BLE ble_hs_mbuf_to_flat...");
...
int BLEManagerImpl::ble_svr_gap_event(struct ble_gap_event * event, void * arg)
{
    ...
    case BLE_GAP_EVENT_ADV_COMPLETE:
        ChipLogProgress(DeviceLayer, "BLE_GAP_EVENT_ADV_COMPLETE event");
        break;
    ...
    case BLE_GAP_EVENT_MTU:
        ChipLogProgress(DeviceLayer, "BLE_GAP_EVENT_MTU = %d channel id = %d", event->mtu.value, event->mtu.channel_id);
        break;
...
void BLEManagerImpl::HandleC3CharRead(struct ble_gatt_char_context * param)
{
    ...
    SuccessOrExit(err);
	ChipLogProgress(DeviceLayer, "BLE os_mbuf_append...");
...
CHIP_ERROR BLEManagerImpl::StartAdvertising(void)
{
    ...
    {
    	ChipLogProgress(DeviceLayer, "BLE set advertising response data...");
	...
	ChipLogProgress(DeviceLayer, "Start BLE gap advertising");
    err = MapBLEError(ble_gap_adv_start(own_addr_type, NULL, BLE_HS_FOREVER, &adv_params, (ble_gap_event_fn *)ble_svr_gap_event, NULL));
    if (err == CHIP_NO_ERROR)
    {
        return CHIP_NO_ERROR;
    }
