...
#include "ExampleCommissionableDataProvider.h"
...
CHIP_ERROR ExampleCommissionableDataProvider::Init(chip::Optional<std::vector<uint8_t>> serializedSpake2pVerifier,
                                                 ...
                                                 chip::Optional<uint32_t> setupPasscode, uint16_t discriminator)
{
    ...
    {
        ChipLogError(Support, "ExampleCommissionableDataProvider didn't get a PASE salt, but got a verifier: ambiguous data");
        return CHIP_ERROR_INVALID_ARGUMENT;
    }
    ...
    if (!havePaseSalt)
    {
        ChipLogProgress(Support, "ExampleCommissionableDataProvider didn't get a PASE salt, generating one.");
...
CHIP_ERROR ExampleCommissionableDataProvider::GetSetupDiscriminator(uint16_t & setupDiscriminator)
{
...
CHIP_ERROR ExampleCommissionableDataProvider::GetSpake2pIterationCount(uint32_t & iterationCount)
{
    VerifyOrReturnError(mIsInitialized == true, CHIP_ERROR_WELL_UNINITIALIZED);
...
CHIP_ERROR ExampleCommissionableDataProvider::GetSpake2pSalt(chip::MutableByteSpan & saltBuf)
{
    VerifyOrReturnError(mIsInitialized == true, CHIP_ERROR_WELL_UNINITIALIZED);
...
CHIP_ERROR ExampleCommissionableDataProvider::GetSpake2pVerifier(chip::MutableByteSpan & verifierBuf, size_t & outVerifierLen)
{
    VerifyOrReturnError(mIsInitialized == true, CHIP_ERROR_WELL_UNINITIALIZED);
...
CHIP_ERROR ExampleCommissionableDataProvider::GetSetupPasscode(uint32_t & setupPasscode)
{
    VerifyOrReturnError(mIsInitialized == true, CHIP_ERROR_WELL_UNINITIALIZED);
