 ...
 *
 * @param[in] nfft number of input samples
 * @return Structure pointer of half-precision floating-point CFFT configuration
 *
 * @b Note:
 *
 ...
 *
 * 2. This function will allocate a temporary buffer used to store the twiddle table, temporary
calculate results and some control flags. If the buffer allocation is not successful the
function will return NULL. The buffer size will depend on the number of
input samples and the detail memory usage please refer to cfft function descriptions.
 */
 ...
 *
 * @param[in] nfft number of input samples
 * @return Structure pointer of half-precision floating-point RFFT configuration
 *
 * @b Note:
 *
 * 1. This function is only supported by the toolchain with the HW FPU support. Also, you need to apply option “-mzfh” both in compiling flags and linking flags for applications that use this function. For details about the linking option“-mzfh”.
